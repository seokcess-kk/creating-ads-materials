...
import { clampLayerOverlaps, copyLimitsForTypography, countCopyChars } from "./copy-limits";
...
function toComposeLayer(
  ...
  return {
    ...
    // 실측 박스에 새(더 긴) 카피가 들어올 수 있어 축소 하한을 넉넉히 — 넘치면 throw로 전량 실패하므로.
    minScale: layer.role === "headline" ? 0.55 : 0.5,
...
export function buildReferenceTextLayers(input: {
  ...
  const complete = (!headline || used.has("headline"))
    ...
    && (!sub || used.has("sub"));
  return complete ? clampLayerOverlaps(result) : [];
}
